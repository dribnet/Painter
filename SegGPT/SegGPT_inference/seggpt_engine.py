...
def inference_image_tom(model, device, img_path, img2_paths, tgt2_paths, out_path):
    res, hres = 448, 448
    ...
    # print("INPUT")
    # print(input_image);
    ones = 255 * np.ones_like(input_image);
    output = Image.fromarray((ones * (output / 255)).astype(np.uint8))
    # output = Image.fromarray((input_image * (0.6 * output / 255 + 0.4)).astype(np.uint8))
    output.save(out_path)
...
def inference_video_tom(model, device, vid_path, num_frames, img2_paths, tgt2_paths, out_path):
    res, hres = 448, 448
